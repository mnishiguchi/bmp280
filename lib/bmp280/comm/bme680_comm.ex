defmodule BMP280.BME680Comm do
  ...
  defp set_power_mode(transport, mode) do
    with {:ok, <<no_change::6, _mode::2>>} <- Transport.I2C.read(transport, @ctrl_meas_register, 1) do
      Transport.I2C.write(transport, @ctrl_meas_register, <<no_change::6, mode::2>>)
    end
  end
  ...
  def set_oversampling(transport) do
    ...
    with :ok <- Transport.I2C.write(transport, @ctrl_hum_register, <<osrs_h>>) do
      Transport.I2C.write(transport, @ctrl_meas_register, <<osrs_t::3, osrs_p::3, mode::2>>)
    end
  end
  ...
  def set_filter(transport) do
    with {:ok, config} <- Transport.I2C.read(transport, @config_register, 1),
  ...
  defp set_gas_status(transport, run_gas) do
    with {:ok, ctrl_gas1} <- Transport.I2C.read(transport, @ctrl_gas1_register, 1),
  ...
  def set_gas_heater_temperature(transport, heater_resistance, heater_set_point \\ 0) do
    Transport.I2C.write(transport, @res_heat0_register + heater_set_point, <<heater_resistance>>)
  end
  ...
  def set_gas_heater_duration(transport, heater_duration, heater_set_point \\ 0) do
    Transport.I2C.write(transport, @gas_wait0_register + heater_set_point, <<heater_duration>>)
  end
  ...
  def set_gas_heater_profile(transport, heater_set_point) do
    with {:ok, ctrl_gas1} <- Transport.I2C.read(transport, @ctrl_gas1_register, 1),
      ...
      Transport.I2C.write(transport, @ctrl_gas1_register, <<no_change::4, heater_set_point::4>>)
    end
  end
  ...
  def read_calibration(transport) do
    with {:ok, coeff3_binary} <- Transport.I2C.read(transport, @coeff3_register, 5),
         {:ok, coeff1_binary} <- Transport.I2C.read(transport, @coeff1_register, 23),
         {:ok, coeff2_binary} <- Transport.I2C.read(transport, @coeff2_register, 14),
  ...
  def read_raw_samples(transport) do
    with :ok <- set_forced_mode(transport),
         ...
         {:ok, pth} <- Transport.I2C.read(transport, @press_msb_register, 8),
         {:ok, gas} <- Transport.I2C.read(transport, @gas_r_msb_register, 2),
  ...
  defp new_data?(transport) do
    {:ok, binary} = Transport.I2C.read(transport, @meas_status0_register, 1)
